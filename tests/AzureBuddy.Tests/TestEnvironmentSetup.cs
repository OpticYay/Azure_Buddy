...
/// <summary>
/// Program.cs reads ConnectionStrings:DefaultConnection and Jwt:SigningKey directly off configuration
/// during its own top-level statements, before WebApplicationFactory's ConfigureWebHost customizations
/// ever get a chance to run - confirmed by reproducing this against a clean checkout: those
/// customizations are only applied once the host is actually built, which happens after Program.cs's
/// own reads. Locally this is invisible because a real (gitignored) appsettings.json supplies both
/// values, but a fresh CI checkout has neither file, so Program.cs throws "Missing
/// ConnectionStrings:DefaultConnection in configuration" before a single test can run.
///
...
internal static class TestEnvironmentSetup
{
    ...
    {
        SetIfAbsent("ConnectionStrings__DefaultConnection", "Server=localhost;Port=3306;Database=unused;Uid=unused;Pwd=unused;");
